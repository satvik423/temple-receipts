...
export async function GET(request: Request) {
  ...
  const workbook = excelReport.kanike(
    receipts,
    { name: settings.name, place: settings.place, phone: settings.phone },
    `Kanike Report - ${period.title}`,
  );

  return excelDownloadResponse(workbook, `kanike-report-${period.filenameSuffix}.xlsx`);
}
